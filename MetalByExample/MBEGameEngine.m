...
@property (nonatomic, strong) MBECubePointLight *redLight;
...
- (instancetype)initWithView:(MTKView *)view size:(CGSize)size
{
	...
	self.view = view;
	NSPanGestureRecognizer *panGR = [[NSPanGestureRecognizer alloc] initWithTarget:self action:@selector(didPanWithTwoFingers:)];
...
{
	const vector_float4 cameraTranslation = {0, 0, 8, 0};
	self.position = cameraTranslation;
	self.rotY = M_PI;
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
	...
	vector_float4 position = self.position;
	float rotY = self.rotY;

	self.position = (vector_float4){0, 0, 8, 0};
	self.rotY = M_PI;
	[self updateWorldToViewMatrix];
...
//
//	CGFloat x = event.locationInWindow.x / CGRectGetWidth(windowFrame) - 0.5;
//	CGFloat y = event.locationInWindow.y / CGRectGetHeight(windowFrame) - 0.5;

//	self.redLight.x = 10 * x;
//	self.redLight.y = 10 * y;
}

- (void)didPanWithTwoFingers:(NSPanGestureRecognizer *)gr
{
//	CGPoint loc = [gr locationInView:gr.view];
////	CGFloat x = loc.x / CGRectGetWidth(self.view.bounds) - 0.5;
//	CGFloat y = loc.y / CGRectGetHeight(self.view.bounds) - 0.5;
//
//	CGPoint translation = [gr translationInView:gr.view];
//	self.redLight.z = 10 * translation.y / CGRectGetHeight(self.view.bounds);
}
